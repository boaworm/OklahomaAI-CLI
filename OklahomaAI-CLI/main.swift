...
//  OklahomaAI-CLI
//
//  Created by Henrik Thorburn on 1/14/24.
//

import Foundation

print("Hello, World - from Deer Classification ML project in Swift!")
...
do {
    ...
    for filePath in imageArray {
        if(filePath.hasSuffix(".JPG")){
            ...
            let deerImage = try DeerClassifierInput(imageAt: url!)
...
func createJsonOutput(observations : [Observation]){
    ...
    do {
        ...
        let url = URL(fileURLWithPath: imageDirPath + "/deerNoDeer.json")
        try json.write(to: url, atomically: true, encoding: .utf8)
        
        //let filepath = Bundle.main.path(forResource: imageDirPath + "/deerNoDeer.json", ofType: nil)
...
func printPrediction(prediction: DeerClassifierOutput){
    let p = getPredictedClass(probabilityMap: prediction.targetProbability)
